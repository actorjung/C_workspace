...
{
    int key = hash(x->name, h->size);   // 검색할 데이터의 해시값
...
{
    int key = hash(x->name, h->size);   // 추가할 데이터의 해시값
    Bucket *p = &h->table[key];         // 현재 선택한 노드
