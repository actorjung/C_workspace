...
int Initialize(ChainHash *h, int size)
{
    ...
    h->size = size;
    for (int i = 0; i < size; i++)  // 모든 버킷을 공백(NULL) 상태로 변경
        h->table[i] = NULL;
...
{
    int key = hash(x->name, h->size);   // 검색하는 데이터의 해시값
...
{
    int key = hash(x->name, h->size);   // 추가하는 데이터의 해시값
    Node *p = h->table[key];            // 현재 선택한 노드
    Node *temp;

    while (p != NULL) {
        if (!strcmp(p->data.name, x->name))     // 이 키는 이미 등록됨
            return 1;                   // 추가 실패
        p = p->next;                    // 다음 노드를 선택
    }
...
{
    int  key = hash(x->name, h->size);  // 삭제하는 데이터의 해시값
    Node *p = h->table[key];            // 현재 선택한 노드
    Node **pp = &h->table[key];         // 현재 선택한 노드에 대한 포인터

    while (p != NULL) {
        if (!strcmp(p->data.name, x->name)) {   // 찾으면
            *pp = p->next;
            free(p);                    // 해제
            return 0;                   // 삭제 성공
        }
